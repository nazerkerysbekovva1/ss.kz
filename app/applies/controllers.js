...
const getUserApplies = async (req, res) => {
    try {
...
const acceptUser = async (req, res) => {
    try {
        ...
        sendMail(user.email, `Вы были приглашены на ваканцию ${vacancy.name}`, `
        Компания: ${company.name}, пригласила вас на вакасию ${vacancy.name}, приходите по адресу ${company.address}
...
const declineUser = async (req, res) => {
    try {
        ...
        sendMail(user.email, `Отказ на ваканцию ${vacancy.name}`, `
        Компания: ${company.name}, к сожелению ваша кондидатура не подхдит на вакансию ${vacancy.name}`)
