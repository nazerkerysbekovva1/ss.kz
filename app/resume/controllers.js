...
const getworkingHistory = async (req, res) => {
    const workingHistory = await WorkingHistory.findByPk(req.params.id);
    res.status(200).send(workingHistory);
}



const getworkingHistoryPost = async (req, res) => {
    const resumeId = req.params.id;
    const workingHistory = await WorkingHistory.findAll({
        where: {
          resumeId,
        },
    });
...
const searchResume = async (req, res) => {
    const options = {}
    const {q, cityId, salary_from, salary_to, salary_type, citizenchip} = req.query
    if(q) {
        options[Op.or] = [
    ...
    if(citizenchip) {
        options.citizenchip = citizenchip;
    }
    ...
    if(salary_from && !salary_to) {
        options.salary = { [Op.gte]: salary_from*1 }  //to convert to int
    }
